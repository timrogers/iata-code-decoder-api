...
/**
 * Creates a prefix-based Map for O(1) lookups of IATA codes and their prefixes.
 *
 * Each key in the map is a lowercase prefix of an IATA code, from length 1 up to
 * the full length of the code. This allows for extremely fast partial and exact
 * matches without linear array scanning.
 *
 * @param objects - The dataset to process (airports, airlines, or aircraft)
 * @returns A Map where keys are prefixes and values are arrays of matching objects
 */
const createPrefixMap = (objects: Keyable[]): Map<string, Keyable[]> => {
  const map = new Map<string, Keyable[]>();

  // Ensure empty string returns all results for backward compatibility (e.g., in /airlines)
  map.set('', objects);

  for (const object of objects) {
    const code = object.iataCode.toLowerCase();
    // Generate all possible prefixes for this IATA code
...
// Pre-process datasets into prefix maps at startup for O(1) lookup performance
...
const filterObjectsByPartialIataCode = (
  ...
  if (partialIataCode.length > iataCodeLength) {
    ...
    // Perform O(1) Map lookup instead of O(N) linear scanning
