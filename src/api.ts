...
import { AIRPORT_INDEX } from './airports.js';
import { AIRLINES, AIRLINE_INDEX } from './airlines.js';
...
function createMcpServer(): Server {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        case 'lookup_airport': {
          const airports = AIRPORT_INDEX.lookup(query);
          return {
            content: [
              {
                type: 'text',
        ...
        case 'lookup_airline': {
          const airlines = AIRLINE_INDEX.lookup(query);
          return {
            content: [
              {
                type: 'text',
        ...
        case 'lookup_aircraft': {
          const aircraft = AIRCRAFT_INDEX.lookup(query);
          return {
            content: [
              {
                type: 'text',
