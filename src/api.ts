...
/**
 * Creates a map of IATA code prefixes to their corresponding objects.
 * This allows for O(1) lookup of objects by their IATA code or prefix.
 */
...
function createMcpServer(): Server {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        case 'lookup_airport': {
          const airports = AIRPORTS_MAP.get(query.toLowerCase()) || [];
          return {
            content: [
              {
                type: 'text',
        ...
        case 'lookup_airline': {
          const airlines = AIRLINES_MAP.get(query.toLowerCase()) || [];
          return {
            content: [
              {
                type: 'text',
        ...
        case 'lookup_aircraft': {
          const aircraft = AIRCRAFT_MAP.get(query.toLowerCase()) || [];
          return {
            content: [
              {
                type: 'text',
