...
describe('IATA Code Decoder API - Integration Tests', () => {
  ...
  describe('CORS', () => {
    it('should not include Access-Control-Allow-Origin header when CORS_ALLOWED_ORIGINS is not set', async () => {
      const response = await app.inject({
        method: 'GET',
        url: '/health',
        headers: {
    ...
    it('should not include CORS headers on preflight when CORS_ALLOWED_ORIGINS is not set', async () => {
      const response = await app.inject({
        method: 'OPTIONS',
        url: '/airports',
        headers: {
