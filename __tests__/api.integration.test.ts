...
describe('IATA Code Decoder API - Integration Tests', () => {
  ...
  describe('CORS', () => {
    it('should include Access-Control-Allow-Origin: * header on regular responses', async () => {
      const response = await app.inject({
        method: 'GET',
        url: '/health',
      });
    ...
    it('should respond to OPTIONS preflight requests with 204 and CORS headers', async () => {
      const response = await app.inject({
        method: 'OPTIONS',
        url: '/airports',
      });
