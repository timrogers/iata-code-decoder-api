    ...
    describe('Airports Endpoint', () => {
        describe('Valid Requests', () => {
            it('should return airports matching partial IATA code', async () => {
                const response = await request(app)
                    .get('/airports?query=LON')
                    .expect(200);
                expect(response.body).toHaveProperty('data');
                expect(Array.isArray(response.body.data)).toBe(true);
                expect(response.body.data.length).toBeGreaterThan(0);
                // Check that all returned airports have IATA codes starting with 'LON'
                response.body.data.forEach((airport) => {
                    expect(airport.iataCode.toLowerCase()).toMatch(/^lon/);
                });
            });
            it('should return specific airport for exact IATA code', async () => {
                ...
                const heathrow = response.body.data.find((airport) => airport.iataCode === 'LHR');
