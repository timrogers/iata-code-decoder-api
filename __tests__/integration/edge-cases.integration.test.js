...
describe('IATA Code Decoder API - Edge Cases and Stress Tests', () => {
    ...
    describe('Performance and Load Testing', () => {
        it('should handle rapid successive requests', async () => {
            const requests = Array.from({ length: 20 }, (_, i) => request(app)
        ...
        it('should maintain performance with complex queries', async () => {
            ...
            const requests = queries.map(query => request(app).get(`/airports?query=${query}`).expect(200));
        ...
        it('should handle stress test with many concurrent connections', async () => {
            ...
            const requests = Array.from({ length: concurrentRequests }, (_, i) => request(app)
                .get(`/health`)
                .expect(200));
            const startTime = Date.now();
            const responses = await Promise.all(requests);
    ...
    describe('Data Consistency Under Load', () => {
        it('should return consistent results under concurrent load', async () => {
            ...
            const requests = Array.from({ length: numRequests }, () => request(app).get(`/airports?query=${query}`).expect(200));
            const responses = await Promise.all(requests);
